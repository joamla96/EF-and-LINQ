...
namespace DAL.Tests.UpdateTests
{
    ...
    public class ConcurrencyTests : BaseTest
    {
        ...
        public void ShouldThrowConcurrencyError()
        {
            ...
            var currentCatName = currentValues[nameof(cat.CategoryName)];
            var databaseValues = entry.GetDatabaseValues();
            var databaseCatName = databaseValues[nameof(cat.CategoryName)];
            Assert.Equal("Foo",originalCatName);
            Assert.Equal("Bar",databaseCatName);
            Assert.Equal("FooBar",currentCatName);
        }
    }
}
